...
- Hunting Hour ↣ {Levii}
    ''')
                    ...
                    await sbb_b.send_message(event.chat_id, f'''خطأ مع {username}
    الخطأ :
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        break
                    else:
                        await sbb_b.send_message(event.chat.id, " اجاك متاح !")
            else:
                pass
            trys += 1
...
- Hunting Hour ↣ {Levii}
    ...
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await sbb_b.send_message(event.chat_id, "تم الانتهاء من التثبيت التلقائي")
